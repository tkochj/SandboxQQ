...
class ExecutePythonTool(SandboxTool):
    ...
    async def run(self, sandbox_root: str, **kwargs) -> str:
        ...
        sandbox_wrapper = f'''
import builtins, os, sys, importlib, gc, io, pathlib
from pathlib import Path
...
def _raise(msg):
    raise PermissionError(f"[沙盒] {{msg}}")

# ── 模块导入拦截 ────────────────
_EXTRA_BLOCKED = {{"importlib", "gc", "ctypes", "subprocess", "socket",
                   "multiprocessing", "shutil", "pty", "fcntl", "resource",
                   "syslog", "codeop", "code", "compileall", "py_compile",
                   "this", "antigravity", "webbrowser", "turtle",
                   "pdb", "profile", "trace", "cProfile"}}

_original_import = builtins.__import__
def _safe_import(name, *args, **kwargs):
    base = name.split(".")[0]
    if base in _EXTRA_BLOCKED:
        _raise(f"模块 '{{name}}' 被禁止导入")
...
# ── 禁用内置危险函数 ────────────────
for _name in ["eval", "exec", "compile", "execfile"]:
    if hasattr(builtins, _name):
        setattr(builtins, _name, lambda *a, _n=_name, **kw: _raise(f"builtins.{{_n}} 被禁止"))

# ── gc 模块全局禁用 ────────────────
if "gc" in sys.modules:
    _gc = sys.modules["gc"]
    for _attr in ["get_referrers", "get_objects", "get_referents",
                  "collect", "set_debug", "set_threshold"]:
        if hasattr(_gc, _attr):
            setattr(_gc, _attr, lambda *a, **kw: _raise(f"gc.{{_attr}} 被禁止"))

# ── io 模块 patch ────────────────
_original_io_open = io.open
def _safe_io_open(file, mode="r", *args, **kwargs):
    safe = _sandbox_resolve(str(file))
    if not safe:
        _raise(f"路径超出沙盒范围: {{file}}")
    return _original_io_open(safe, mode, *args, **kwargs)
io.open = _safe_io_open

# ── pathlib 模块 patch ────────────
_original_path_open = pathlib.Path.open
def _safe_path_open(self, mode="r", *args, **kwargs):
    safe = _sandbox_resolve(str(self))
    if not safe:
        _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_open(pathlib.Path(safe), mode, *args, **kwargs)
pathlib.Path.open = _safe_path_open

def _safe_path_read_text(self, *args, **kwargs):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_read_text(pathlib.Path(safe), *args, **kwargs)

def _safe_path_write_text(self, *args, **kwargs):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_write_text(pathlib.Path(safe), *args, **kwargs)

def _safe_path_read_bytes(self, *args, **kwargs):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_read_bytes(pathlib.Path(safe), *args, **kwargs)

def _safe_path_write_bytes(self, *args, **kwargs):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_write_bytes(pathlib.Path(safe), *args, **kwargs)

def _safe_path_iterdir(self):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_iterdir(pathlib.Path(safe))

def _safe_path_glob(self, pattern):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_glob(pathlib.Path(safe), pattern)

def _safe_path_rglob(self, pattern):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_rglob(pathlib.Path(safe), pattern)

def _safe_path_stat(self):
    safe = _sandbox_resolve(str(self))
    if not safe: _raise(f"路径超出沙盒范围: {{self}}")
    return _original_path_stat(pathlib.Path(safe))

def _safe_path_exists(self):
    safe = _sandbox_resolve(str(self))
    return _original_path_exists(pathlib.Path(safe)) if safe else False

def _safe_path_absolute(self):
    return pathlib.Path(_SANDBOX_ROOT) / str(self) if hasattr(self, 'parts') else pathlib.Path(_SANDBOX_ROOT)

def _safe_path_resolve(self):
    safe = _sandbox_resolve(str(self))
    return pathlib.Path(safe) if safe else pathlib.Path(_SANDBOX_ROOT)
...
pathlib.Path.rename = _safe_path_rename
pathlib.Path.resolve = _safe_path_resolve

# ── fileinput patch ────────────────
try:
    import fileinput as _fi_mod
    _orig_fi_input = _fi_mod.input
    def _safe_fi_input(files=None, *args, **kwargs):
        if files:
            safe_files = []
            for f in files:
                s = _sandbox_resolve(str(f))
                if not s: _raise(f"路径超出沙盒: {{f}}")
                safe_files.append(s)
            return _orig_fi_input(safe_files, *args, **kwargs)
        return _orig_fi_input(*args, **kwargs)
    _fi_mod.input = _safe_fi_input
except Exception:
    pass
...
try:
    ...
    for _attr in ["mkstemp", "mkdtemp", "mkfile", "NamedTemporaryFile",
                  "TemporaryFile", "SpooledTemporaryFile", "TemporaryDirectory"]:
        if hasattr(_tf, _attr):
            setattr(_tf, _attr, lambda *a, _n=_attr, **kw: _raise(f"tempfile.{{_n}} 被禁止"))
except Exception:
    pass
...
_os_blocked = [
    ...
    "kill", "killpg", "fork", "forkpty", "chroot", "setuid", "setgid",
    "seteuid", "setegid", "setreuid", "setregid", "setresuid", "setresgid",
    "getuid", "geteuid", "getgid", "getegid", "getpid", "getppid",
    "getlogin", "uname", "sysconf",
]
for _fn in _os_blocked:
    if hasattr(os, _fn):
        def _blocked(*args, _name=_fn, **kw):
            _raise(f"os.{{_name}} 被禁止")
        setattr(os, _fn, _blocked)

# ── 禁止 reload ────────────────
try:
    importlib.reload = lambda m, _n="reload": _raise("importlib.reload 被禁止")
except Exception:
    pass

# ── sys 模块限制 ────────────────
if hasattr(sys, "_getframe"):
    sys._getframe = lambda *a, **kw: _raise("sys._getframe 被禁止")
if hasattr(sys, "_current_frames"):
    sys._current_frames = lambda *a, **kw: _raise("sys._current_frames 被禁止")
...
# ── chdir ────────────────
def _chdir_blocked(*a, **kw):
    _raise("禁止切换工作目录")
os.chdir = _chdir_blocked

# ── pathlib.rename 沙盒约束 ────────────────
def _safe_path_rename(self, target):
    safe_src = _sandbox_resolve(str(self))
    safe_dst = _sandbox_resolve(str(target))
    if not safe_src or not safe_dst:
        _raise(f"路径超出沙盒范围: {{self}} -> {{target}}")
    return _original_path_rename(pathlib.Path(safe_src), pathlib.Path(safe_dst))
pathlib.Path.rename = _safe_path_rename
